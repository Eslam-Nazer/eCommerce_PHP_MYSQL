...
<div class="container">
    ...
    if(isset($_GET['name']) && $_GET['name'] != null ) {
        ...
        <div class="row gx-5">
            <?php 
            $stmt = $con->prepare("SELECT * FROM items WHERE tags LIKE '%{$tag}%'");
            $stmt->execute();
            $items = $stmt->fetchAll();
            // echo '<pre>';
            // print_r($items);
    ...
    <?php } else {
        $errorMsg = '<div class="alert alert-danger mt-4" role="alert"><i class="fa-solid fa-circle-xmark"></i> You Must choose tag name</div>';
